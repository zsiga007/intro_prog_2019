## Represent the opponents (and their checkers etc.) with
## numbers, because we represent everything with numbers.
RED = 0
BLACK = 1

## We have to decide in which direction each opponent will proceed.
RED_HOME = 0

## The bar, where "captured" checkers are waiting.
bar = [0, 0]

## The board, initial setup:
points_number = [2, 0, 0, 0, 0, 5,
                 0, 3, 0, 0, 0, 5,
                 5, 0, 0, 0, 3, 0,
                 5, 0, 0, 0, 0, 2]
points_colour = [BLACK, 0, 0, 0, 0, RED,
                 0, RED, 0, 0, 0, BLACK,
                 RED, 0, 0, 0, BLACK, 0,
                 BLACK, 0, 0, 0, 0, RED]

## Strings for drawing the board:
colours = ["", ""]
colours[RED] = "R"
colours[BLACK] = "B"

################
## Board printing
################
## The printed representation of each point will occupy
## three characters.  The first and the last character
## will be a space (" ") if the point contains no checker
## at all.  Otherwise, the third character will be the
## character representing the colour of the checkers at
## the point.  The first character will be non-space only
## if the point contains more than 9 checkers.

def print_point( checkers, colour_string ):
    if checkers == 0:
        print( " 0 ", end="" )
    elif checkers < 10:         # one-digit number
        ## add extra space before:
        print( " ", checkers, colour_string, sep="", end="" )
    else:                       # two-digit number
        print( checkers, colour_string, sep="", end="" )

## This procedure prints the board
## (missing: the bar is not printed).
## The board is represented by twelve consecutive
## lines, containing two points each (so we
## rotate the usual view of the board by 90 degrees).

def draw_board( points_number, points_colour ):
    ## Number of spaces between the two columns
    ## ("column separator"):
    column_sep = "        "
    ## We will print 12 lines, with two points each:
    for i in range( 12 ):
        ## Number of checkers at the two points
        ## in this row:
        checkers1 = points_number[11 - i]
        checkers2 = points_number[12 + i]
        print_point( checkers1, colours[points_colour[11 - i]] )
        print( column_sep, end="" )
        print_point( checkers2, colours[points_colour[12 + i]] )
        ## Don't forget the newline at the end of each line:
        print()

## Print board:
draw_board( points_number, points_colour )

## Whoever starts the game
next_player = RED

## We don't know yet how to roll a dice.
roll = 4

## Tell the user what number has been thrown.
## (In actual fact, we roll with two dice,
## but that can wait.)
print( "Roll =", roll )

## Tell the users who is next:
if next_player == RED:
    print( "Next player is red." )
else:
    print( "Next player is black." )

## Ask next player what move they want to make:
move_read = False
while not move_read:
    ## Missing: make sure the input is actually a number.
    starting_point = int( input( "starting point of move: " ) )
    endpoint = int( input( "endpoint of move: " ) )
    ## Make sure the given user actually has checkres on
    ## the starting point:
    if points_number[starting_point] > 0 and \
       points_colour[starting_point] == next_player:
        ## Make sure the user can move to the endpoint.
        ## First case: the endpoint is unoccupied or
        ## contains checkers of the given user:
        if points_number[endpoint] == 0 or \
           points_colour[endpoint] == next_player:
            ## Make the move:
            points_number[starting_point] = \
                points_number[starting_point] - 1
            points_number[endpoint] = \
                points_number[endpoint] + 1
            points_colour[endpoint] = next_player
            ## To stop the iteration (loop):
            move_read = True
        ## Second case: the endpoint is occupied by
        ## one checker that belong to the oppenent:
        elif points_number[endpoint] == 1 and \
             ( ( next_player == RED and
                 points_colour[endpoint] == BLACK ) or
               ( next_player == BLACK and
                 points_colour[endpoint] == RED ) ):
            ## Make the move:
            points_number[endpoint] = 1
            points_colour[endpoint] = next_player
            ## To stop the iteration (loop):
            move_read = True
        else:                   # bad endpoint
            print( "You cannot move there, try again!" )
    else:                       # bad starting point
        print( "You cannot move from there, try again!" )

## Print board again:
draw_board( points_number, points_colour )
